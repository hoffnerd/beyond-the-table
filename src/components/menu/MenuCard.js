 ...
 * @prop [backImage=menu_neutral.png] - string that represents the file name of the image to be displayed on the back side of the menu card. It defaults to "menu_neutral.png" if not provided.
 ...
 */
const MenuCard = ({id, link, display, description, frontImage, backImage="menu_neutral.png", columnSizes={lg:4, md:6, sm:12}, disabledReason=false, largeTitle=false, pathSoFar=null}) => {
                ...
                <Link href={disabledReason ? "#" : link ? `/${link}` : pathSoFar ? `${pathSoFar}/${id}` : `/${id}`} className="menu-card">
                    <div className="menu-card-image-grid">
                        ...
                        <div className={`menu-card-image menu-card-image-grid__side ${disabledReason ? `menu-card-image-grid__side--${disabledReason}` : "menu-card-image-grid__side--back"}`}>
                            <Image
                                alt={disabledReason ? `/images/menu/menu_${disabledReason}.png` : `/images/menu/${backImage}`}
                                src={disabledReason ? `/images/menu/menu_${disabledReason}.png` : `/images/menu/${backImage}`}
                                fill
                                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
